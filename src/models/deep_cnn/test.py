"""
Evaluation script for the trained DeepCNN model.
...
-------------------------------------------
    python -m src.models.deep_cnn.test
    python -m src.models.deep_cnn.test --checkpoint path/to/best_model.pth
"""
import os
import sys
import argparse
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate DeepCNN on the test set.')
    ...
    model = build_deep_cnn(num_classes=num_classes, dropout=cfg['model']['dropout']).to(device)
