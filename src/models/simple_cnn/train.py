"""
Training script for SimpleCNN on crop disease detection.

Trained from scratch (no pretrained weights). Single-phase training.

Grid search findings (simplecnn.ipynb):
    Both Adam lr=0.1 and SGD lr=0.1 failed — LR too high.
    Default set to Adam lr=0.001.

Usage (run from the project root directory)
-------------------------------------------
    python -m src.models.simple_cnn.train

Outputs saved to checkpoints/simple_cnn/
...
def main():
    ...
    # StepLR matches the notebook scheduler (step_size=5, gamma=0.5)
