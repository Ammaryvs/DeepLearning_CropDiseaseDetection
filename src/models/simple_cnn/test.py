"""
Evaluation script for the trained SimpleCNN model.
...
-------------------------------------------
    python -m src.models.simple_cnn.test
    python -m src.models.simple_cnn.test --checkpoint path/to/best_model.pth
"""
import os
import sys
import argparse
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate SimpleCNN on the test set.')
    ...
    model = build_simple_cnn(num_classes=num_classes, dropout=cfg['model']['dropout']).to(device)
